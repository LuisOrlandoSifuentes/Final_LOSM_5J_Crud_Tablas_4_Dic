...
def registrarCliente(request):
    if request.method == "POST":
        Cliente.objects.create(
            ...
            productos_adquiridos=request.POST["txtProductosAdquiridos"],
        )
...
def editarCliente(request):
    if request.method == "POST":
        ...
        cliente.tipo_de_pago = request.POST["txtTipoPago"]
        cliente.fecha_nac = request.POST["dateFechaNac"]
        cliente.productos_adquiridos = request.POST["txtProductosAdquiridos"]
        cliente.save()
    return redirect("cliente")
